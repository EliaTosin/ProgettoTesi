...
# match path with labels (int) and classes (string)
def getImgPool(path):
    only_dirs = [ name for name in os.listdir(path) if 
                 os.path.isdir(os.path.join(path, name)) ]
    only_dirs.sort()
...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if image.shape[0] != 3: #convert grayscaled e RGBA images in RGB
        ...
        preprocess2 = Compose([ # 2nd preprocess to adapt to the pretrained resnet
...
print (f'Num of images in test dataset: {test_data.__len__()}')
...
# checking gpu accelaration (CUDA)
...
# check if loaded on device
print(next(net.parameters()).device)

best_accuracy = 0
for epoch in range(NUM_OF_EPOCHS):  # loop on dataset
    net.train()
    running_loss = 0.0
    for i, data in enumerate(train_dl):
        # picking images from dataloader
        inputs, labels = data
        ...
        optimizer.zero_grad()
        outputs = net(inputs) # getting preds
        ...
        skip = 50
        if i > 0  and i % skip == 0:    # print training loss every [skip] batches
    ...
    # end of the epoch, computing test loss and accuracy
    with torch.no_grad():
        net.eval()
        test_loss = 0
        test_acc = 0
        ...
        # if we have an improvement in the accuracy, we have a better model
...
print(f"Best acc: {best_accuracy:.5f}")

#saving the last model (the one obtained at the last epoch)
